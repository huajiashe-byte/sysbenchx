...
sysbench.cmdline.options = {
   ...
   table_columns = 
      {"table_columns per table", 4},
   trx_read_nums = 
      {"The Number of read count per transaction", 1},
   trx_write_nums = 
      {"The Number of write(delete/insert/update) count per transaction", 1}
}
...
function cmd_prepare()
   ...
   if sysbench.opt.table_columns < 4 then
      print("FATAL: table_columns needs to be greater than 4. Please set a larger value and try again!")
      os.exit(0)
   end
...
local pad_value_template = "###########-###########-###########-" ..
   "###########-###########"
-- 1 groups, 5 characters
local cx_value_template = "#####"

function get_cx_value()
   return sysbench.rand.string(cx_value_template)
end
...
--      sysbench.opt.create_table_options)
  ...
  "pad CHAR(60) DEFAULT '' NOT NULL," 
   ...
   sql_extra = ""
   if sysbench.opt.table_columns > 4 then
      for i = 5, sysbench.opt.table_columns do
         sql_extra = sql_extra .. "c" .. i .. " char(5) DEFAULT '', "
      end
   end

   sql_con = sql_con .. sql_extra .. "%s (id)" .. ") %s %s"
   
   query = string.format(sql_con,
   ...
   for i = 1, sysbench.opt.table_size do
      ...
      --                         c_val, pad_val)
      --end
      query_con = sysbench.rand.default(1, sysbench.opt.table_size) .. ",'" .. c_val .. "', '" .. pad_val .. "'"  
      query = "("
      col_con = ""
      if (sysbench.opt.auto_inc) then
         query = query .. query_con
         for j = 5, sysbench.opt.table_columns do
            col_con = col_con .. ",'" .. get_cx_value() .. "'"
	 end
      query = query .. col_con
      else
         query = query .. i .. query_con 
	 for j = 5, sysbench.opt.table_columns do
            col_con = col_con .. ",'" .. get_cx_value() .. "'"
	 end
	 query = query .. col_con
...
function prepare_delete_inserts()
   if (sysbench.opt.table_columns > 4) then
      ins_head = "INSERT INTO sbtest%u (id, k, c, pad"
      ins_mid = ") VALUES (?, ?, ?, ?"
      ins_con_col = ""
      ins_con_val = ""
      for j = 5, sysbench.opt.table_columns do
         ins_con_col = ins_con_col .. ', c' .. j
         ins_con_val = ins_con_val .. ', ?'
         table.insert(stmt_defs["inserts"], {t.CHAR, 5})
      end
      stmt_defs["inserts"][1] = ins_head .. ins_con_col .. ins_mid .. ins_con_val .. ")"
   end
...
function execute_delete_inserts()
   ...
   for i = 1, sysbench.opt.delete_inserts do
      ...
      if sysbench.opt.table_columns > 4 then
         for j = 5, sysbench.opt.table_columns do
            param[tnum].inserts[j]:set_rand_str(get_cx_value())
         end
      end
